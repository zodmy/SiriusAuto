...
export function ProductCard({ id, name, price, imageUrl, description, category, inStock = true, discount, rating, reviewCount, href, onAddToCart, isLoading = false, actions, className = '' }: ProductCardProps) {
    ...
    <Card padding='none' className={`group hover:shadow-lg transition-shadow duration-200 ${className}`}>
        ...
        <div className='aspect-square relative overflow-hidden rounded-t-lg bg-gray-100'>
          {imageUrl ? (
            <Link href={productHref}>
              <Image src={imageUrl} alt={name} fill className='object-cover group-hover:scale-105 transition-transform duration-200' sizes='(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw' />
            </Link>
          ) : (
            <Link href={productHref}>
      ...
      <div className='p-4 space-y-3'>
        ...
        {actions || onAddToCart ? (
          <div className='pt-2'>
            {actions || (
              <Button onClick={onAddToCart} disabled={!inStock || isLoading} isLoading={isLoading} size='sm' fullWidth>
