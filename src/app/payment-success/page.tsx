...
function PaymentSuccessContent() {
  ...
  const checkPaymentStatus = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Якщо платіж не успішний, перенаправляємо на сторінку помилки
        if (data.isFailed) {
  ...
  }, [orderId, router, setIsLoading, setIsCheckingStatus, setPaymentData, setError]);

  useEffect(() => {
    document.title = 'Оплата завершена - Sirius Auto';
  }, []);
  useEffect(() => {
    if (!orderId) {
      router.push('/');
            ...
            <div className='flex'>
              <div className='ml-3'>
                ...
                <ul className='text-sm text-green-600 mt-2 list-disc list-inside'>
                  <li>Ви отримаєте email-підтвердження на вказану адресу</li>
                  <li>Наш менеджер зв&apos;яжеться з вами для уточнення деталей доставки</li>
                  <li>Ви можете відстежувати статус замовлення в особистому кабінеті</li>
